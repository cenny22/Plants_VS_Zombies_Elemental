...
if (zumbis_nascidos >= total_fase) {
    ...
    tempo_min = 3;
    tempo_max = 5;
} else if (zumbis_mortos >= 10) {
    zumbi_para_criar = obj_zumbi_arbusto;
} else if (zumbis_mortos >= 5) {
    var chance = irandom(99);
    if (chance < 50) zumbi_para_criar = obj_zumbi_cone;   
    else if (chance < 90) zumbi_para_criar = obj_zumbi_basico; 
    else zumbi_para_criar = obj_zumbi_balde;  
} else {
    if (zumbis_nascidos >= 3) zumbi_para_criar = obj_zumbi_arbusto;
}
