...
if (qtd_quadrados > 0) {
    ...
    if (zumbis_nascidos >= total_fase) {
        ...
        if (chance < 40) zumbi_para_criar = obj_zumbi_galinheiro;
        else if (chance < 65) zumbi_para_criar = obj_zumbi_arbusto;
        else if (chance < 85) zumbi_para_criar = obj_zumbi_gorila;
        else zumbi_para_criar = obj_zumbi_tarzan;
    } else if (zumbis_nascidos >= 10 || global.zumbis_mortos >= 10) {
        tempo_min = 3;
        tempo_max = 6;
        var chance = irandom(99);
