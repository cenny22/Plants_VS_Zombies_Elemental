...
if (variable_global_exists("fase_iniciada") && global.fase_iniciada) {
    if (variable_instance_exists(id, "planta_associada") && planta_associada != noone) {
        ...
        // SÓ DEIXA SELECCIONAR SE NÃO ESTIVER EM COOLDOWN E TIVER SÓIS SUFICIENTES
        if (!em_cooldown && global.sois >= custo) {
            ...
            if (global.planta_selecionada == planta_associada) {
                ...
                // Guarda qual card ativou a seleção para acionar o cooldown depois
                global.card_selecionado = id; 
            }
        }
    }
} 
